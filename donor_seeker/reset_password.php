     ...
     if($_POST)
    {
        ...
        if(count($error_msg) == 0)
        {
          ...
          if(!$sql)
          {
            ...
            $error_msg2['sql'] = "There was an error.";
            exit();
          }
          else
          {
                ...
                <div class="card-body">
                    <form method="POST" action="">
                        <div class="form-group row">
                            ...
                            <?php }
                                elseif(!empty($error_msg2)) { ?>
                                  <div class="error2" data-wrong2="<?= $error_msg2; ?>"> 
                                  </div>
                            <?php } ?>
                        </div>
                                ...
                                <?php 
                                  if(isset($_GET['reset']))
                                  {
                                    if($_GET['reset'] == "success")
                                    {
                                       ...
                                       ?>
                                       <div class="success" data-sent="<?= $_GET['reset']; ?>"> 
                                       </div>
                                <?php
                                    }
                                    elseif($_GET['reset'] == "error")
                                    {
                                      echo "<div class='error'>E-mail was not sent.Please try again...</div>";
                                      ?>
                                      <div class="not-sent" data-notSent="<?= $_GET['reset']; ?>"> 
                                      </div>
                                <?php
                                    }
                                  }
                   ...
                   const wrong2 = $('.error2').data('wrong2')
                    if(wrong2){
                      Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'There was an error.Please check out...'
                          })
                   }
                   const notSent = $('.not-sent').data('notSent')
                    if(notSent){
                      Swal.fire({
                            icon: 'error',
                            title: 'Oops...',

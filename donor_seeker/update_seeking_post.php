<?php 
   ...
   if($_POST)
   {
        ...
        if(count($error_msg) == 0)
        {
            ...
            if(!is_numeric($Division) || !is_numeric($District) || !is_numeric($Sub_District_or_Police_Station) || !is_numeric($village_or_area))
            {
              $division = $_POST['division'];
              $district = $_POST['district'];
        ...
        /* Update by id of donor_seeker_post */
         ...
         $sql=mysqli_query($connection , $query); 
            if($sql)
            {
              ...
              if($token == "not change") //location was not changed
                {
                   $seeker_id = $row['seeker_id'];

                 if($_SESSION['id'] == $seeker_id) // user him/herself
                   {
                     header("location:seeking_post_of_user.php?post_update=post_update");
                     ob_end_flush();
                   }
                 else // admin
                   {
                     header("location:admin_taskbar.php?post_update=post_update");
                     ob_end_flush();
                   }
                }
              elseif($token == "change") //location was changed
                {
                   $id = $_GET['id'];// post id
            ...
            {
                /* Not Update */
                header("location:update_seeking_post.php?error=error&id=".$id);
                ob_end_flush();
            }
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 style="text-align: center;">Update Donor Seeker Post</h5>
                </div>
                <div class="card-body">
                            ...
                            <?php
                             ...
                             ?>
                              ...
                              <?php 
                                    ...
                                    <div class="error_msg" data-error_msg="<?= $error_msg; ?>"> 
                                    </div>
                              <?php }
                                ?>
                                 ...
                                 <script>
                                  const updated = $('.update').data('updated')
                                    ...
                                    const error_msg = $('.error_msg').data('error_msg')
                                    if(error_msg){
                                      Swal.fire({
                                            icon: 'error',
                                            title: 'Oops...',
